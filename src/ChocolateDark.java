public class ChocolateDark {

    private final Stock stock;

    public ChocolateDark(Stock stock) {
        this.stock = stock;
    }
    ...
    public void make() throws InterruptedException {
        ...
        if (stock.getWater() < needWater || stock.getSugar() < needSugar || stock.getPowderChoco() < powderChoco || stock.getCups() < needCup || stock.getSpoons() < needSpoon) {
            System.out.println(Color.RED + "Le chocolat noir est momentanément indisponible" + Color.RESET);
            Thread.sleep(800);
            return;
        }

        System.out.println("Votre chocolat noir est en cours de préparation. \n Veuillez patienter \n");
